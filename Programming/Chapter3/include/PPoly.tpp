...
template <typename Real> PPoly<Real>::PPoly() {}
...
template <typename Real> PPoly<Real>::PPoly(const PPoly<Real> &other) {
  this->_t = other._t;
  this->_coeffs = other._coeffs;
}

template <typename Real> int PPoly<Real>::findInterval(Real x) const {
  /**
...
template <typename Real> Real PPoly<Real>::operator()(Real x) const {
  /**
...
template <typename Real> Real PPoly<Real>::derivative(Real x, int n) const {
  ...
  // Eigen::VectorXd tmp = Eigen::Map<const Eigen::VectorXd>(&_coeffs[idx][n], m
  // + 1);
...
template <typename Real> Real PPoly<Real>::integral(Real a, Real b) const {
  /**
...
template <typename Real> PPoly<Real> &PPoly<Real>::operator=(const PPoly<Real> &other) {
  // construct it
  if (this != &other) {
    this->_t = other._t;
    this->_coeffs = other._coeffs;
