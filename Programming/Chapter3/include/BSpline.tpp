...
BSpline<Real>::BSpline(
    std::vector<Real> coeffs, std::vector<Real> t, const int n, const int check) :
    _coeffs(coeffs),
    _n(n) {
  _basis.resize(n + 1);
  for (int i = 0; i <= n; i++) {
...
BSpline<Real>::BSpline(const BSpline &other) :
    ...
    _total_basis(other._total_basis), _derivative_basis(other._derivative_basis),
    _n(other._n) {}
 ...
 * @return int
 */
template <typename Real> int BSpline<Real>::get_interval(const Real x) const {
  int begin_idx = _n - 1;
...
template <typename Real> Real BSpline<Real>::operator()(const Real x) {
...
template <typename Real> std::vector<Real> BSpline<Real>::get_basis(const Real x) {
  int interval = get_interval(x);
  if (interval == -1) {
  ...
  for (int k = 1; k <= _n; ++k) {
    ...
    {
      Real left = (j == 0) ? Real(0)
                           : (x - _t[interval - k + j]) /
                                 (_t[interval + j] - _t[interval - k + j]);
      Real right = (j == k) ? Real(0)
                            : (_t[interval + j + 1] - x) /
...
template <typename Real>
BSpline<Real> &BSpline<Real>::operator=(const BSpline<Real> &other) {
...
template <typename Real> std::vector<Real> BSpline<Real>::get_coeffs() const {
  return _coeffs;
}

template <typename Real> std::vector<Real> BSpline<Real>::get_t() const { return _t; }

template <typename Real> int BSpline<Real>::get_n() const { return _n; }

// std::vector<std::vector<Real>>
template <typename Real> void BSpline<Real>::get_total_basis(const Real x) {
  int interval = get_interval(x);
  if (interval == -1) {
    return;
  }
...
template <typename Real>
Real BSpline<Real>::cal_derivative_basis(
    const int interval, const int j, const int degree, const int n) {
  if (n == 0) {
    ...
    // _t[interval+j-_n]) * cal_derivative_basis(interval, j-1, degree-1, n-1);
    // Real right = static_cast<Real>(degree) / (_t[interval+j+degree-_n+1] -
    // _t[interval+j+1-_n]) * cal_derivative_basis(interval, j, degree-1, n-1);
    Real left = 0.0, right = 0.0;
    if (j > 0) {
      left = (static_cast<Real>(degree) /
              (_t[interval + j + degree - _n] - _t[interval + j - _n])) *
...
template <typename Real>
std::vector<Real> BSpline<Real>::basis_derivative(const Real x, const int n) {
  int interval = get_interval(x);
  if (interval == -1) {
...
template <typename Real> Real BSpline<Real>::derivative(const Real x, const int n) {
  if (n > _n) {
    return 0.0;
  } else if (n < 0) {
