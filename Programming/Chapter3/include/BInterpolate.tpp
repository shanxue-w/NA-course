...
BInterpolate<N, Real>::BInterpolate(
  ...
  if (check == 1) {
    if (!std::is_sorted(_t.begin(), _t.end())) {
      ...
      std::sort(
          idx.begin(), idx.end(), [&](int i, int j) { return t[i] < t[j]; });
...
template <int N, typename Real>
void BInterpolate<N, Real>::interpolate(
    const std::vector<Real> &t,
    const std::vector<Real> &y,
  ...
  if (method == 0) {
    ...
    int t_size = t.size();
    // Eigen::SparseMatrix<Real, Eigen::RowMajor> A(t_size + N - 1, t_size + N -
    // 1); Eigen::MatrixXd A(t_size+N-1, t_size+N-1); Eigen::VectorXd
    // b(t_size+N-1);
    Eigen::Matrix<Real, Eigen::Dynamic, Eigen::Dynamic> A(
        t_size + N - 1, t_size + N - 1);
    Eigen::Matrix<Real, Eigen::Dynamic, 1> b(t_size + N - 1);
    ...
    for (int i = 0; i < N - 1; i++) {
      ...
      for (int j = 0; j < N; ++j) {
        // triplets.push_back(Eigen::Triplet<Real>(i, j, diff_basis[j]));
        A(i, j) = diff_basis[j];
      }
    ...
    for (int i = 0; i < t_size; ++i) {
      ...
      if (i == 0) {
        ...
        for (int j = 0; j < N; ++j) {
          // triplets.push_back(Eigen::Triplet<Real>(N - 1 + i, i + j, basis[j +
          // 1]));
          A(N - 1 + i, i + j) = basis[j + 1];
        }
      }
    ...
    // if (solver.info() != Eigen::Success)
    // {
    //     std::cerr << "Failed to solve the system of equations" << std::endl;
    // }
...
template <int N, typename Real>
Real BInterpolate<N, Real>::operator()(const Real x) {
  return _bspline(x);
}

template <int N, typename Real>
BSpline<Real> BInterpolate<N, Real>::getBSpline() const {
  return _bspline;
}

template <int N, typename Real>
Real BInterpolate<N, Real>::derivative(const Real x, const int n) {
