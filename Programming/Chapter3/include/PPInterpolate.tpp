...
template <int N, typename Real>
void PPInterpolate<N, Real>::interpolate(
    const std::vector<Real> &t, // nodes
    const std::vector<Real> &y, // values
  ...
  if (method == 0) {
    ...
    for (int i = 0; i < t_size - 2; i++) {
      ...
      for (int k = 0; k < N - 1; k++) {
        for (int j = k + 1; j < N - 1; j++) {
          A(k, j) =
              CMatrix(k, j) * tmp_lists(j - k - 1); // tmp_lists(j-i) = dx^{j-i}
        }
...
template <int N, typename Real>
Real PPInterpolate<N, Real>::operator()(Real x) const {
  return poly(x);
}

template <int N, typename Real>
PPoly<Real> PPInterpolate<N, Real>::getPoly() const {
  return poly;
}
