...
Curve<N, Real>::BSplineBall() const
{
    ...
    this->CalculateThetaPhi(theta, phi);
    // CalculateThetaPhi(theta, phi);
...
Curve<N, Real>::BSplineBallProj() const
{
    Real r = SQRT(_xy[0][0] * _xy[0][0] + _xy[1][0] * _xy[1][0] + _xy[2][0] * _xy[2][0]);
    Real x_0, y_0, z_0;
    this->FindRandomPoint(x_0, y_0, z_0);
    int t_size = _t.size();
...
template <int N, typename Real>
void
Curve<N, Real>::CalculateThetaPhi(std::vector<Real> &theta, std::vector<Real> &phi) const
{
    int flag = 0;
    // initial radius
    Real r = SQRT(_xy[0][0] * _xy[0][0] + _xy[1][0] * _xy[1][0] + _xy[2][0] * _xy[2][0]);
...
template <int N, typename Real>
bool
Curve<N, Real>::isPointInSet(Real x, Real y, Real z) const
{
...
template <int N, typename Real>
void
Curve<N, Real>::FindRandomPoint(Real &x_0, Real &y_0, Real &z_0) const
{
    Real r = SQRT(_xy[0][0] * _xy[0][0] + _xy[1][0] * _xy[1][0] + _xy[2][0] * _xy[2][0]);
